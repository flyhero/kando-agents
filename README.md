...
agent 退出后，任务进入「待验收」，不管退出码是多少：交互式 agent 做完和中途被叫停都以 0 退出，所以退出码只作提示（非 0 或未知时显示「异常退出」「已中断」）。终端右侧的**检查器**列出 agent 从分支起点到现在改了什么：提交记录、每个文件的增删行数（包括还没提交和没加入 git 的），点开文件能看 diff；「分支」标签显示 worktree 的分支和未提交的改动。选中待验收的任务时它会自动打开，工具栏的检查器按钮可以随时开关。依赖任务的改动不会算进来：起点取分支创建时的提交。看过结果后三选一：
...
会话头部的检查器按钮打开右侧的**检查器**（有项目时才有）：「本次会话以来的提交」以新建会话时记下的各项目 HEAD 为起点；「还没提交的改动」和当前 HEAD 比较，包括没加入 git 的文件，点开能看 diff。项目目录是和你共用的，所以这里也可能包含你自己的改动。旧会话没有记录起点，只显示还没提交的改动。
...
状态栏右下角的终端按钮（或 Ctrl+`）打开右侧的终端面板，里面是你自己的登录 shell，可以开多个标签。新终端的工作目录跟着当前选中的内容走：任务的 worktree、会话的项目，都没有时是 home。shell 和 agent 一样由 daemon 托管，隐藏面板或关闭窗口都不会结束它；关闭标签或在 shell 里 `exit` 才会结束。面板可以拖动调宽，也可以最大化。
