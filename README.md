...
```
未执行 pending ──执行──▶ 执行中 running ──退出──▶ 已执行 done ──继续──▶ 执行中（同一个 worktree）
                                                           │
...
已执行的任务不会回到「未执行」。对结果不满意时二选一：

- **继续**：在原来的 worktree 和分支上开一个新的 agent 会话，它先总结上次做了什么，再按你的新要求接着改。
...
细化时 agent 会知道每个依赖任务的状态：已执行的只告诉它改动在哪（已经进了当前代码，或还在某个分支上，可以用 git 查看），让它直接读代码；未执行的附上计划（每个最多 2000 字）。需要完整内容时，它可以用同一个 MCP 服务里的只读工具 `read_task_details` 读取依赖链上任意任务的详情。
...
- **依赖**：依赖的任务全部「已执行」后才能执行，不能形成循环。同一个 Git 仓库里恰好有一个依赖留下了分支时，新 worktree 从那个分支拉出，直接用上前置任务的改动；prompt 里也会列出依赖任务和它们的分支。
...
- **验收流程**：已完成的任务查看 diff、合并分支、清理 worktree
